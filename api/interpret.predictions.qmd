...
| average_by        | str \| list \| bool \| None | The covariates we would like to average by. The passed covariate(s) will marginalize over the other covariates in the model. If True, it averages over all covariates in the model to obtain the average estimate. Defaults to `None`. | `None`     |
...
|----------|--------------|---------------------------------------------------------------------------------|
| cap_data | pd.DataFrame | A DataFrame with the result of `create_predictions_data` and model predictions. |
