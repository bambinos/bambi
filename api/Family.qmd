...
| `link`       | typing.Union\[str, typing.Dict\[str, typing.Union\[str, bambi.families.link.Link\]\]\] | The link function that's used for every parameter in the likelihood function. Keys are the names of the parameters and values are the link functions. These can be a ``str`` with a name or a ``bambi.families.Link`` instance. The link function transforms the linear predictors. | _required_ |
...
| `posterior` | xarray.Dataset | The xarray dataset that contains the draws for all the parameters in the posterior. It must contain the parameters that are needed in the distribution of the response, or the parameters that allow to derive them. | _required_ |
...
| Type             | Description                                                            |
|------------------|------------------------------------------------------------------------|
| xarray.DataArray | A data array with the draws from the posterior predictive distribution |
