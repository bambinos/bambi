...
| link       | str or dict of str to (str or Link) | The link function that's used for every parameter in the likelihood function. Keys are the names of the parameters and values are the link functions. These can be a `str` with a name or a `bambi.families.Link` instance. The link function transforms the linear predictors. | _required_ |
...
| data      | pd.DataFrame or None | A data frame with values for the predictors and the response on which the model's log-likelihood function is evaluated. If omitted, the original dataset is used.                                                    | _required_ |
| kwargs    | dict                 | Parameters that are used to get draws but do not appear in the posterior object or other configuration parameters. For instance, the 'n' in binomial models and multinomial models.                                  | `{}`       |
