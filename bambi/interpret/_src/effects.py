...
class PredictiveDifferences:
    ...
    def set_variable_values(self, draws: dict) -> np.ndarray:
        ...
        if self.kind == "slopes" and self.variable.user_passed:
            ...
            self.variable.values = self.variable.values.reshape(
                2, self.variable.passed_values.size
            )
    ...
    def get_slope_estimate(
        ...
        predictive_difference = (predictive_difference / eps).rename(
    ...
    def get_estimate(
        ...
        for idx, _ in enumerate(self.variable.values):
            ...
            select_draw = response_transforms(
                idata.posterior[self.response.name_target].sel(
                    {self.response.name_obs: mask}
                )
            )
            select_draw = select_draw.assign_coords(
                {
                    self.response.name_obs: np.arange(
                        len(select_draw.coords[self.response.name_obs])
                    )
                }
            )
        ...
        for idx, pair in enumerate(pairwise_variables):
            ...
            difference_mean[f"estimate_{idx}"] = predictive_difference.mean(
                ("chain", "draw")
            )

            if self.use_hdi:
                difference_bounds[f"estimate_{idx}"] = az.hdi(
                    predictive_difference, prob
                )
            else:
                difference_bounds[f"estimate_{idx}"] = predictive_difference.quantile(
                    q=(self.response.lower_bound, self.response.upper_bound),
                    dim=("chain", "draw"),
                )
    ...
    def get_summary_df(self, response_dim: np.ndarray) -> pd.DataFrame:
        ...
        if len(self.variable.values) > 2 and self.kind == "comparisons":
            summary_df = self.preds_data.drop(
                columns=self.variable.name
            ).drop_duplicates()
            covariates_cols = summary_df.columns
            contrast_values = list(
                itertools.combinations(self.variable.values.flatten(), 2)
            )
        ...
        # scenario 3 & 4
            ...
            for idx, _ in enumerate(self.variable.values):
                ...
                wrt[f"draw_mask_{idx}"] = self.preds_data[mask][
                    self.variable.name
        ...
        summary_df.insert(
            ...
            self.response.upper_bound_name,
            self.estimate.higher,
        )
...
def slopes(
    ...
    if isinstance(wrt, dict):
        if len(wrt) > 1:
            raise ValueError(
                f"Only one predictor can be passed to 'wrt'. {len(wrt)} were passed."
            )
    ...
    if not isinstance(wrt, dict) and conditional is None:
        if is_categorical_dtype(model.data[wrt_name]) or is_string_dtype(
            model.data[wrt_name]
        ):
